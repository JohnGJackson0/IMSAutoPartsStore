...
public class SelectOrderViewModel extends AndroidViewModel {
    ...
    public LiveData<PagedList<Order>> getAllFinishedOrders() {
        return new LivePagedListBuilder<>(
                mRepository.getAllFinishedOrders(), 15)
                .build();
    }

    public void removeAllOnSpecialty(long orderNumber) {
        mRepository.setOnSpecialtyOrder(orderNumber);
    }
}
